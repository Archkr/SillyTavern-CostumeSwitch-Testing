...
// ### BUG FIX: START ###
// The single unified attribution regex was brittle. It has been split into three
// more robust, independent regexes.

function buildPostQuoteAttributionRegex(patternList, verbString) {
...
function buildPreQuoteAttributionRegex(patternList, verbString) {
    const e = (patternList || []).map(parsePatternEntry).filter(Boolean); if (!e.length) return null;
    const names = e.map(x => `(?:${x.body})`).join("|");
    const verbs = processVerbsForRegex(verbString);
    if (!verbs) return null;
...
function findAllMatches(combined, regexes, settings, quoteRanges) {
    ...
    // ### BUG FIX: START ###
    // Use the three separate, robust attribution regexes instead of one brittle one.
...
jQuery(async () => {
    ...
    // ### BUG FIX: START ###
    // The single attributionRegex has been replaced by three more specific ones.
    let nameRegex, speakerRegex, postQuoteAttributionRegex, preQuoteAttributionRegex, voiceAttributionRegex, directActionRegex, possessiveRegex, vocativeRegex, vetoRegex;
    function recompileRegexes() { 
        try { 
            ...
            // Compile the three new, separate attribution regexes
            postQuoteAttributionRegex = buildPostQuoteAttributionRegex(effectivePatterns, attributionVerbs);
            preQuoteAttributionRegex = buildPreQuoteAttributionRegex(effectivePatterns, attributionVerbs);
    ...
    registerSlashCommand("scene", (args) => { const debugMode = (args[0] || '').trim().toLowerCase() === 'debug'; const ctx = getContext(); const lastMessage = ctx.chat.slice().reverse().find(msg => !msg.is_user && msg.mes); if (!lastMessage) { toastr.warning("Could not find the last AI message to analyze."); return; } const tempProfile = saveCurrentProfileData(); 
        const tempRegexes = {}; 
        try { 
            ...
            tempRegexes.postQuoteAttributionRegex = buildPostQuoteAttributionRegex(effectivePatterns, tempProfile.attributionVerbs);
            tempRegexes.preQuoteAttributionRegex = buildPreQuoteAttributionRegex(effectivePatterns, tempProfile.attributionVerbs);
