...
test("normalizePatternSlot accepts string and regex slot inputs", () => {
    const stringSlot = normalizePatternSlot("Alice");
    assert.equal(stringSlot.name, "Alice");
    assert.equal(stringSlot.aliases, undefined);

    const regexSlot = normalizePatternSlot(/Alic?e/i);
    assert.equal(regexSlot.name, "/Alic?e/i");
    assert.equal(regexSlot.aliases, undefined);
});
