...
export async function load(url, context, defaultLoad) {
    ...
    if (url === "node:mock/slash") {
        return {
            format: "module",
            source: `const store = globalThis.__mockSlashStore || (globalThis.__mockSlashStore = { commands: [] });\nexport const executeSlashCommandsOnChatInput = async (command) => {\n    store.commands.push(command);\n    if (typeof store.onExecute === "function") {\n        await store.onExecute(command);\n    }\n};\nexport const registerSlashCommand = () => {};`,
            shortCircuit: true,
        };
    }
