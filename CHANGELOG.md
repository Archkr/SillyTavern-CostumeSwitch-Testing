...
- **Streaming detection cadence.** Live generation now runs throttled, incremental detection passes against the streaming buffer so rankings refresh during the stream without overwhelming the matcher.
- **Streaming token diagnostics.** Stream ingestion now logs raw tokens, normalized chunks, message keys, and buffer lengths to make debugging live buffer growth easier.
...
- **Streaming detection caching.** Streaming detection now skips duplicate passes when the message key and buffered content have not changed, reducing redundant analytics updates during long generations.
...
### Fixed
- **Streaming speaker metadata.** Snapshot-based detection now prefixes assistant message names when the chat log omits inline speaker labels, restoring character switches for group chats that store the speaker separately.
- **Streaming pronoun handoff.** Streaming match scoring now favors explicit name detections over inherited pronoun subjects, preventing early pronoun locks from blocking later character switches.
- **Legacy pattern slot normalization.** String or regex character slots now normalize into valid pattern entries so older profiles keep detecting characters instead of compiling empty pattern lists.
- **Scene control center streaming roster.** The scene panel now binds to the active streaming message state even when the stored scene snapshot lags behind, restoring live roster detections during assistant generation.
- **Streaming key resolution.** Stream detections now anchor to the live chat log and SillyTavern's streaming processor output instead of token payloads, preventing mismatched message keys during in-flight generations.
- **Streaming key metadata fallback.** Stream key detection now reads message metadata IDs (and timestamps when needed) so live generations stop defaulting to the first character when the chat log omits message keys.
- **Streaming snapshot tokens.** Token payloads that include the full streamed message now update the live buffer immediately, keeping detections active even when the host doesn't expose a streaming processor snapshot.
- **Stream snapshot fallback.** Streaming detections now fall back to token buffering when snapshot polling cannot resolve assistant text, preventing silent stalls when the chat log only updates on completion.
- **Streaming buffer key normalization.** Streaming detection now normalizes message keys before buffering so each stream reuses the correct message state instead of defaulting to the first character in the list.
- **Costume folder anchoring.** Costume switching now distinguishes full folder paths from subfolder-only overrides so `/costume` targets the intended character.
...
- **Streaming message key telemetry.** Added debug instrumentation for stream key resolution and a streaming-processor ID fallback so live detections can initialize even when token metadata is missing.
...
- **Scene roster rehydration on render.** Streaming completions now remap temporary message keys to final IDs and rehydrate roster state so the scene panel stays in sync.
